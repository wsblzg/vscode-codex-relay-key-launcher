...
- 图形界面读取 `C:\Users\DAS\.codex-relay\accounts.json`。
...
图形界面会读取 `C:\Users\DAS\.codex-relay\accounts.json`，勾选一个或多个账号后点击“打开选中窗口”，每个账号会打开一个独立 `CODEX_HOME` 的 VSCode 窗口。
...
```powershell
.\Start-CodexRelayVSCode.ps1 -List
```

用当前 preferred 账号打开 `D:\项目`：

```powershell
.\Start-CodexRelayVSCode.ps1 -Workspace "D:\项目"
```

用当前 preferred 账号打开，并按 `provider` 共享会话状态：

```powershell
.\Start-CodexRelayVSCode.ps1 -Workspace "D:\项目" -ShareStateByProvider
```

指定账号打开：

```powershell
.\Start-CodexRelayVSCode.ps1 -Account api-freemodel-dev-17 -Workspace "D:\项目"
```

用 preferred 的下一个账号打开：

```powershell
.\Start-CodexRelayVSCode.ps1 -Next -Workspace "D:\项目"
```

一次打开多个账号窗口：

```powershell
.\Start-CodexRelayVSCode.ps1 -Accounts api-freemodel-dev-17,api-freemodel-dev-18,api-freemodel-dev-19 -Workspace "D:\项目"
```
...
```powershell
.\Start-CodexRelayVSCode.ps1 -Account api-freemodel-dev-17 -PrepareOnly
```
...
```powershell
.\Start-CodexRelayVSCode.ps1 -Account api-freemodel-dev-17 -UseSeparateExtensionsDir
```
